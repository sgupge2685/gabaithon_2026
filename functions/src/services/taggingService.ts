...
export async function generatePhotoTags(
  ...
  try {
    ...
    if (
      ...
      const response =
        await fetch(imageInput);

      if (!response.ok) {
        throw new Error(
          `画像URLの取得に失敗しました: ${response.status}`
        );
      }
    ...
    // --------------------------------------------------
    // ② 渡されたデータが「パソコン内のファイルパス」の場合
    // tests/images/xxx.jpg
    // --------------------------------------------------
    ...
    // --------------------------------------------------
      ...
      if (headerMatch) {
        ...
        if (
          ...
          console.error(
            `✕ 未対応のBase64画像形式です: ${headerType}（対応形式: jpg, jpeg, png, heic, heif, webp）`
          );
...
4. タグの数:
   写真の情報量に応じて3〜8個程度を出力してください。
   無理に増やす必要はありません。

5. タグの品質:
   同じ意味のタグを重複して出さないでください。
   日本語で簡潔なタグを出力してください。
`;
